...
public class ProductController : ControllerBase
{
    ...
    public ProductController(ICatalogServiceClient catalogServiceClient)
    {
    ...
    public async Task<ActionResult<Product>> GetProductAsync([Required][FromQuery] Guid productId)
    {
        try
        {
            Product product = await m_catalogServiceClient.GetProductAsync(productId);
            return Ok(product);
        }
    ...
    {
        Product[] productArray = (await m_catalogServiceClient.GetProductsAsync(productIds)).ToArray();
    ...
    {
        Product[] productArray = (await m_catalogServiceClient.GetAllProductsAsync()).ToArray();
        return Ok(productArray);
    }
    ...
    {
        ProductPageResponse pageResponse = await m_catalogServiceClient.GetPagedProductsAsync(pageSize, pageIndex, title, enabled);
    ...
    {
        ProductRelationship[] productRelationshipArray = (await m_catalogServiceClient.GetProductRelationshipsAsync(productId, includeReverseRelationships)).ToArray();
        return Ok(productRelationshipArray);
    }
    ...
    {
        Product createdProduct = await m_catalogServiceClient.CreateProductAsync(product);
    ...
    public async Task<ActionResult> UpdateProductAsync([Required][FromBody] Product product)
    {
        try
        {
            await m_catalogServiceClient.UpdateProductAsync(product);
            return Ok();
        }
        catch (ApiException apiException) when (apiException.StatusCode == StatusCodes.Status404NotFound)
        {
    ...
    public async Task<ActionResult> DeleteProductAsync([Required][FromQuery] Guid productId)
    {
        try
        {
            await m_catalogServiceClient.DeleteProductAsync(productId);
            return Ok();
        }
        catch (ApiException apiException) when (apiException.StatusCode == StatusCodes.Status404NotFound)
        {
